...
@Component({
  ...
  providers: [
    ...
    DataService,
    {
      ...
      useExisting: forwardRef(() => CronGeneratorComponent),
      multi: true,
    },
  ],
})
export class CronGeneratorComponent implements OnInit, OnChanges, OnDestroy, ControlValueAccessor {
  ...
  constructor(private dataService: DataService, private injector: Injector, private formBuilder: FormBuilder) {
  ...
  ngOnInit() {
    this.baseFrequency$ = this.cronJobsForm.get('baseFrequency').valueChanges.pipe(
